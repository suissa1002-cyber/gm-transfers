...
- ‫‏slug **עברי** / תווים לא-אנגליים → ⚠️ **פלוט את ה-permalink המלא האמיתי כמו-שהוא. אל תקצר ואל תמציא alias `gm-`/tinyurl** — ל-`api-create.php` אין הרשאת alias מותאם (הפרמטר `&alias=` מתעלם), ושם בדוי = קישור 404 (קרה 30/06/2026, SanDisk 512GB). **השרת מקצר slug עברי ל-TinyURL אמיתי אוטומטית** ב-`wa._shorten_he_product_links` (בתוך `_meta_send_text`, בנקודת השליחה) — אתה לא צריך לעשות כלום.‬
