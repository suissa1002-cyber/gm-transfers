...
COL_TASKNUM    = "numeric_mm40m0ej"  # global unique task number (auto-filled)
...
TYPE_STOCK_CHECK  = 0    # בדיקת מלאי ללקוח מול ספק
...
TYPE_LABELS = {
    ...
    TYPE_STOCK_CHECK:  "בדיקת מלאי ללקוח מול ספק",
}

# Reverse maps (label text → index) + group → agent, for board-wide tooling
PRIORITY_BY_LABEL = {v: k for k, v in PRIORITY_LABELS.items()}
TYPE_BY_LABEL     = {v: k for k, v in TYPE_LABELS.items()}
GROUP_TO_AGENT    = {gid: name for name, gid in AGENT_GROUPS.items()}
AGENT_HEB = {
...
class MondayTasksClient:
    ...
    def create_task(
        ...
        # Assign the next global task number immediately (best-effort). The
        # hourly numberer (number_unnumbered_tasks) also catches anything
        # created manually in the Monday UI, so this is never the only path.
        if result.get("id"):
            try:
                result["task_number"] = self._assign_task_number(result["id"])
            except Exception as e:
                logger.warning(f"task numbering failed (non-fatal): {e}")
    ...
    # ══════════════════════════════════════
    # Global task numbers ("טפל במשימה N")
    # ══════════════════════════════════════

    def _fetch_all_board_numbers(self) -> list[tuple[str, int | None]]:
        """Return (item_id, number-or-None) for EVERY item on the board."""
        rows, cursor = [], None
        while True:
            if cursor:
                q = ('query($c:String!){next_items_page(limit:200,cursor:$c){cursor '
                     'items{id column_values(ids:["%s"]){text}}}}' % COL_TASKNUM)
                page = self._query(q, {"c": cursor}).get("next_items_page", {})
            else:
                q = ('query($b:[ID!]!){boards(ids:$b){items_page(limit:200){cursor '
                     'items{id column_values(ids:["%s"]){text}}}}}' % COL_TASKNUM)
                page = self._query(q, {"b": [str(self.board_id)]})["boards"][0]["items_page"]
            for it in page.get("items", []):
                cv = it.get("column_values") or []
                txt = cv[0]["text"] if cv and cv[0].get("text") else None
                rows.append((it["id"], int(txt) if txt else None))
            cursor = page.get("cursor")
            if not cursor:
                break
        return rows

    def _set_task_number(self, item_id: int | str, number: int) -> None:
        self._query(
            "mutation($b:ID!,$i:ID!,$c:String!,$v:String!){"
            "change_simple_column_value(board_id:$b,item_id:$i,column_id:$c,value:$v){id}}",
            {"b": str(self.board_id), "i": str(item_id),
             "c": COL_TASKNUM, "v": str(number)},
        )

    def _assign_task_number(self, item_id: int | str) -> int:
        """Give one item the next free global number (used on create)."""
        nums = [n for _, n in self._fetch_all_board_numbers() if n is not None]
        nxt = (max(nums) + 1) if nums else 1
        self._set_task_number(item_id, nxt)
        return nxt

    def _fetch_board_items_full(self) -> list[dict]:
        """Every board item with name, group, number, priority, type, status."""
        cols = f'"{COL_TASKNUM}","{COL_PRIORITY}","{COL_TYPE}","{COL_STATUS}"'
        out, cursor = [], None
        while True:
            if cursor:
                q = ('query($c:String!){next_items_page(limit:200,cursor:$c){cursor '
                     'items{id name group{id} column_values(ids:[%s]){id text}}}}' % cols)
                page = self._query(q, {"c": cursor}).get("next_items_page", {})
            else:
                q = ('query($b:[ID!]!){boards(ids:$b){items_page(limit:200){cursor '
                     'items{id name group{id} column_values(ids:[%s]){id text}}}}}' % cols)
                page = self._query(q, {"b": [str(self.board_id)]})["boards"][0]["items_page"]
            for it in page.get("items", []):
                cv = {c["id"]: c["text"] for c in it.get("column_values", [])}
                numtxt = cv.get(COL_TASKNUM)
                out.append({
                    "id": it["id"], "name": it["name"],
                    "group_id": (it.get("group") or {}).get("id", ""),
                    "number": int(numtxt) if numtxt else None,
                    "priority": cv.get(COL_PRIORITY) or "",
                    "type": cv.get(COL_TYPE) or "",
                    "status": cv.get(COL_STATUS) or "",
                })
            cursor = page.get("cursor")
            if not cursor:
                break
    ...
    def number_unnumbered_tasks(self, notify: bool = True) -> int:
        """Assign global numbers to every item lacking one — whether created via
        the API or manually in the Monday UI. Sends a Telegram alert per newly
        numbered (i.e. manually-created) task. Run on a schedule. Returns count."""
        items = self._fetch_board_items_full()
        cur_max = max((it["number"] for it in items if it["number"] is not None), default=0)
        missing = sorted((it for it in items if it["number"] is None),
                         key=lambda x: int(x["id"]))
        for offset, it in enumerate(missing, start=1):
            num = cur_max + offset
            self._set_task_number(it["id"], num)
            if notify:
                try:
                    agent = GROUP_TO_AGENT.get(it["group_id"], self.agent_name)
                    self._send_telegram_task_alert(
                        item_id=it["id"], task_name=it["name"],
                        priority_idx=PRIORITY_BY_LABEL.get(it["priority"]),
                        type_idx=TYPE_BY_LABEL.get(it["type"]),
                        task_number=num,
                        agent_label=AGENT_HEB.get(agent, agent),
                    )
                except Exception as e:
                    logger.warning(f"manual-task alert failed (non-fatal): {e}")
        return len(missing)

    def send_open_tasks_summary(self) -> int:
        """Telegram digest of all OPEN tasks (not done), grouped by agent, with
        their global numbers. Returns the number of open tasks."""
        items = self._fetch_board_items_full()
        open_items = [it for it in items
                      if it["status"] in ("לא התחיל", "בעבודה")]
        if not open_items:
            self._telegram_send("📋 <b>סיכום משימות פתוחות</b>\n\nאין משימות פתוחות 🎉")
            return 0

        # group by agent, ordered by AGENT_GROUPS order
        by_agent: dict[str, list[dict]] = {}
        for it in open_items:
            agent = GROUP_TO_AGENT.get(it["group_id"], "?")
            by_agent.setdefault(agent, []).append(it)

        lines = [f"📋 <b>סיכום משימות פתוחות</b> — {len(open_items)} פתוחות", ""]
        for agent in list(AGENT_GROUPS.keys()) + ["?"]:
            tasks = by_agent.get(agent)
            if not tasks:
                continue
            lines.append(f"<b>{AGENT_HEB.get(agent, agent)}</b>")
            for it in sorted(tasks, key=lambda x: (x["number"] or 9999)):
                mark = "🔵" if it["status"] == "בעבודה" else "⚪"
                lines.append(f"  {mark} <b>#{it['number']}</b> {it['name'][:46]}")
            lines.append("")
        self._telegram_send("\n".join(lines).strip())
        return len(open_items)

    def get_task_by_number(self, number: int | str) -> dict | None:
        """Resolve a global task number to its task (board-wide). Lets a human
        say 'agent, handle task N'. Returns the parsed task, or None."""
        target = int(number)
        for iid, n in self._fetch_all_board_numbers():
            if n == target:
                return self.get_task(iid)
        return None
    ...
    def _send_telegram_task_alert(
        ...
        ‫agent_label: ‏עוקף את שם הסוכן (לשימוש בממספר board-wide על קבוצות אחרות).‬
        """
        if agent_label is None:
            agent_label = AGENT_HEB.get(self.agent_name, self.agent_name)
        ...
        header = (f"📌 <b>משימה #{task_number} ל-{agent_label}</b>"
                  if task_number is not None
                  else f"📌 <b>משימה חדשה ל-{agent_label}</b>")
        lines = [
            header,
            "",
            f"🏷️ <b>{task_name}</b>",
        ...
        lines.append("")
        if task_number is not None:
            lines.append(f'💬 לטיפול אמרו: "{agent_label}, טפל במשימה {task_number}"')
    ...
    def _telegram_send(self, text: str) -> None:
        """Send an HTML message to Asi's private Telegram tasks chat."""
        ...
        logger.info(f"[{self.agent_name}] Telegram message sent → chat {TELEGRAM_TASKS_CHAT_ID}")
