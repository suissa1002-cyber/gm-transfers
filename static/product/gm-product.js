...
(function ($) {
  ...
  function applyPreorder() {
    ...
    $i.removeClass('oos').addClass('gm-pre')
      .html(ico + 'מכירה מוקדמת · ' + (GM_PRE.date ? 'צפי אספקה החל מ-' + GM_PRE.date : 'צפי אספקה בהמשך'));
  }
  ...
  function extractShort() {
    ...
    if ($raw.length) {
      ...
      $raw.find('strong').each(function () {
        ...
        /* שורת תשלום/פרומו ≠ פסקת המוצר — "חדש ! שלמו בהוראת קבע / תשלומים…"
           (iPhone Air) נתפסה כפסקת השיווק ופסקת המוצר האמיתית נפלה לקופסה. */
        if (/תשלומים|הוראת קבע/.test(t)) return;
  ...
  function buildTrust(warranty) {
    ...
    /* ⚠️ "משלוח חינם" אינו נגזרת של מחיר בלבד. מוצרים במחלקת "משלוח כבד"
       (כיסאות גיימינג, רמקולים גדולים) מגיעים ישירות מהמחסן ומוחרגים
       מהמשלוח החינם — 17 מוצרים הציגו הבטחה שהקופה סותרת, ולקוחה נטשה
       עגלה בגלל זה (מיכל, 07/08/2026). מחלקת המשלוח מגיעה מהתבנית
       (data-shipclass); בהיעדרה נשמרת ההתנהגות הישנה.
       ⚠️ הרף הוא 499 ולא 500 — כך מוגדר בוורדפרס ("מעל 499"). */
    var NO_FREE = { 'heavy-products': 1 };
    var scls = ($('.gm-pdp-wrap').attr('data-shipclass') || '').trim();
    var free = price >= 499 && !NO_FREE[scls];
    var cubes = [
      { t: free ? 'משלוח חינם' : 'משלוח רגיל',
        s: NO_FREE[scls] ? 'נשלח ישירות מהמחסן · עלות בקופה'
                         : (free ? '1–6 ימי עסקים' : '1–6 ימי עסקים · חינם מעל ₪499') },
  ...
  /* ⚠️ שורת הצבעים חייבת להיות **ילד ישיר של הכרטיסיה**: בתוך עמודת הטקסט
   * הצרה היא נשברה לשתי שורות ונכנסה לאזור ה-+ (אסי, 09/08). התוסף מרנדר
   * אותה בתוך הטקסט, ולכן מזיזים אותה כאן — בלי לגלגל גרסת תוסף. */
  function gmAdPicksOut() {
    $('.gm-ad-card > .gm-ad-txt .gm-ad-picks').each(function () {
      $(this).appendTo($(this).closest('.gm-ad-card'));
    });
  }
  $(function () { gmAdPicksOut(); });
