...
# 1) 1 יח': סטאר0 גן1 סיטי2 עדהלום2 → גן→אתר(מילוי) + סיטי→סטאר(השלמה)
passed &= _check("1 יח' (סטאר ריק)",
                 _run({STAR: 0, GAN: 1, CITY: 2, ADH: 2}, 1),
                 [(GAN, SITE, 1, False), (CITY, STAR, 1, True)])
...
# 4) סטאר יחידה אחרונה → סטאר ממלא ומתרוקן → השלמה לסטאר מסיטי
passed &= _check("סטאר יחידה אחרונה → השלמה",
