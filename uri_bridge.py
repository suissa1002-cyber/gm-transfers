...
- 🌐 **קישורים** — כל קישור בשורה **נפרדת ופשוטה**, בלי 🔗 או טקסט לפניו באותה שורה
  ...
  השרת מקצר slug עברי ל-TinyURL אמיתי אוטומטית לפני השליחה; slug אנגלי נשלח ישיר.
...
def build_feedback_prompt(feedback: str) -> str:
    """ערוץ פידבק פנימי (קונסולה→אלה): אסי כותב לה תלונה/תיקון/רעיון והיא ממיינת בעצמה —
    ידע/התנהגות → [LESSON] (נקלט בשרת כטיוטה לאישור בתפריט 💡); חסר קוד/יכולת → בלוק
    [DEV] עם פרומפט מוכן לסשן Claude Code. הפלט מוצג לאסי בלבד — לעולם לא ללקוח."""
    pb = fetch_playbook()
    return f"""את אלה — סוכנת שירות הלקוחות של Green Mobile. זו שיחה **פנימית** עם אסי (הבעלים) — לא לקוח!
אסי נותן לך משוב עליך: תלונה, תיקון או רעיון לשיפור. מייני אותו למסלול הנכון וענו לו קצר, חם וענייני.

## המשוב של אסי:
{feedback}

## הפלייבוק הנוכחי שלך (ההתנהגות שלך בשיחות נשלטת על ידו):
{pb or '(ריק)'}

## שלושת המסלולים — בחרי אחד:
1. **ידע / התנהגות / ניסוח** — אפשר לתקן בהנחיה בפלייבוק (מה לענות, איך לנסח, מדיניות):
   כתבי לאסי בשורה-שתיים מה הבנת ושהצעת לקח ממתינה לאישורו בתפריט 💡, וסיימי בשורה נפרדת:
   [LESSON] <הלקח כהנחיה כללית אחת, בלי פרטים מזהים>
2. **חסר קוד / יכולת** — דורש שינוי בקוד (יכולת חדשה, באג טכני, אינטגרציה):
   הסבירי בשורה-שתיים למה זה עניין של קוד ולא של הפלייבוק, ואז כתבי פרומפט מוכן להדבקה
   בסשן Claude Code בתוך הסימונים הבאים (בשורות משלהם):
   [DEV]
   <פרומפט עצמאי לגמרי בעברית: מי שיקרא אותו לא מכיר את השיחה הזו. תיאור הבעיה,
   ההתנהגות הרצויה, והיכן בקוד להתחיל (למשל agents/transfers/wa_bot.py / main.py /
   uri_bridge.py) אם את יודעת. אפשר להציץ בקבצי הריפו כדי לדייק נתיבים.>
   [/DEV]
3. **לא ברור** — שאלי את אסי שאלה ממוקדת אחת להבהרה (בלי [LESSON] ובלי [DEV]).

חשוב: אל תבטיחי לבצע שינויים בעצמך — את לא יכולה לשנות קוד. אל תשלחי שום דבר ללקוחות."""
...
def process(job: dict):
    ...
    if job.get("source") == "learn":
        ...
        return
    # ── 💬 פידבק פנימי מאסי (מהקונסולה): מיון לפלייבוק / פרומפט-קוד — לעולם לא ללקוח ──
    # fast=False בכוונה: המודל המלא + cwd=ROOT (יכולה להציץ בריפו לנתיבי קבצים מדויקים).
    if job.get("source") == "feedback":
        # ניסיון בודד (בלי retry): timeout+retry היו חוצים את חלון ה-requeue_stuck
        # (10 דק') → ריצה כפולה. כשל → אסי פשוט שולח שוב מהצ'אט.
        ok, text, _sid = run_claude(build_feedback_prompt(job["question"]), timeout_s=360)
        answer = (text or "").strip() if ok else "סליחה, תקלה רגעית אצלי — שלח שוב עוד רגע 🙏"
        try:
            requests.post(f"{BASE}/api/uri-bridge/answer", headers=H,
                          json={"id": jid, "answer": answer,
                                "status": "done" if ok else "error"}, timeout=20)
        except Exception as e:  # noqa: BLE001
            log.error("feedback answer post failed #%s: %s", jid, e)
        log.info("feedback job #%s -> %s (%d chars)", jid, "done" if ok else "error", len(answer))
        return
