...
def handle(phone: str, text: str, mtype: str = "text", reply_id: str = "", wamid: str = ""):
    ...
    if low and low not in _GREETINGS:
        ...
        if _is_closing(low):
            ...
            return
        # שאלת שעות פעילות ('אתם פתוחים?', 'שעות פעילות', 'open now') → מענה ישיר.
        # תיקון 30/06: 'פתוחים עכשיו' נחטף קודם לחיפוש מוצר → התאים ל-Open Ear.
        if _is_hours_intent(low):
            _answer_hours(phone)
            return
...
# מילות-שיח מקדימות ("סבבה אני צריכה אייפון...") — מוסרות מתחילת שאילתה לפני
# חיפוש והד ("מצאתי X תוצאות ל'...'"). רק תחילית — שאר המשפט נשאר כמו-שהוא.
# (כשל 16/07: "סבבה אני צריכה אייפון 17 פרו מקס 1 טרה" חופש כמשפט שלם והוצג ללקוחה.)
_FILLER_LEAD = {"סבבה", "אוקיי", "אוקי", "טוב", "אז", "היי", "שלום", "אהלן",
                "אני", "אנחנו", "צריך", "צריכה", "צריכים", "רוצה", "רוצים",
                "מחפש", "מחפשת", "מחפשים", "מעוניין", "מעוניינת", "בבקשה",
                "אפשר", "תביא", "תביאי", "תראה", "תראי", "לי", "לנו", "לראות",
                "לקנות", "להזמין", "בדוק", "בדקי", "תבדוק", "תבדקי", "אם", "יש", "לכם"}


def _strip_filler(text: str) -> str:
    """מסיר מילות-שיח מתחילת שאילתה ('סבבה אני צריכה אייפון' → 'אייפון').
    אם כל המשפט מילות-שיח — מחזיר את המקור (לא מרוקנים שאילתה)."""
    words = (text or "").split()
    i = 0
    while i < len(words) and words[i].strip("?!,.").lower() in _FILLER_LEAD:
        i += 1
    rest = " ".join(words[i:]).strip()
    return rest or (text or "")


def _is_attr_followup(low: str) -> bool:
    w = _strip_filler((low or "").strip())
    if w in _COLORS:
        return True
    if w.startswith("בצבע") or w.startswith("צבע "):
        return True
    if _re.fullmatch(r"\d{2,4}\s*(gb|tb|ג'יגה|גיגה|טרה|ג״ב)?", w, _re.I):
        return True
    # ספרה בודדת רק עם יחידת נפח ("1 טרה", "2TB") — "1" לבד = בחירה מרשימה, לא תכונה
    if _re.fullmatch(r"\d\s*(gb|tb|ג'יגה|גיגה|טרה|ג״ב)", w, _re.I):
        return True
    return False
...
_CLOSE_TOK = {"תודה", "רבה", "הבנתי", "מעולה", "אוקיי", "אוקי", "אוקייי", "סבבה",
              ...
              # אישור קבלה = סגירה ('תודה רבה קבלתי' → אורי שלח 'בודק עבורך' ונפל, 25/06)
              "קבלתי", "קיבלתי", "קבלתיי", "קיבלתיי", "התקבל", "מצוינת", "מעולה!",
...
_REPAIR_VERB_W = ("להחליף", "החלפת", "החלפה", "מחליפים", "לתקן", "תיקון", "תיקונים",
                  "מתקנים", "מתקן", "מתקנת", "תיקני", "שבור", "שבורה", "נשבר", "סדוק",
                  "התקלקל", "מקולקל", "דפוק", "לא עובד", "לא דולק", "נשרט", "מת ", "מתה")
...
def _is_repair_quote_intent(low: str) -> bool:
    ...
    if not has_part:
        return False
    if any(v in s for v in _REPAIR_VERB_W):   # פועל-תיקון מפורש ('להחליף סוללה', 'מתקנים')
        return True
    has_phone = any(w in s for w in _PHONE_HINT)
    # צורך-תיקון על דגם טלפון ('מתקנים אייפון 8 צריך מסך וסוללה') — 'צריך/צריכה + חלק +
    # דגם טלפון' = תיקון, לא מוצר. דורש דגם טלפון כדי לא לתפוס 'צריך מסך למחשב' (שכשל
    # 08/07: הבוט החזיר קטלוג מוצרים במקום מחירון תיקון — למידת אלה).
    if ("צריך" in s or "צריכה" in s or "צריכים" in s) and has_phone:
        return True
...
def _ask_uri(phone, question, wamid="") -> bool:
    ...
    # נקבה (אלה = שם נקבה) — לשון נקבה, בלי 'בודק/ת'.
    if not _uri_engaged(phone):
        wa.send_text(phone, "כמה רגעים, בודקת עבורך 🔎")
...
def _new_order_results(phone, query):
    ...
    import main
    # ניקוי מילות-שיח מהתחילית — גם החיפוש מדויק יותר וגם ההד ללקוח לא מצטט
    # את המשפט השלם ("מצאתי 5 תוצאות ל'סבבה אני צריכה...'").
    query = _strip_filler(query)
...
# שאלת שעות פעילות. מנוסח צר בכוונה כדי לא לתפוס מוצרים ('אוזניות פתוחות'/'Open Ear'):
# דורש הקשר של 'אתם/עכשיו/היום/שעות', לא 'פתוח' לבד.
_HOURS_INTENT_RE = _re.compile(
    r"שעות\s*(ה?פעילות|פתיחה|עבודה)|מה\s+השעות|באיזה\s+שעות|"
    r"מתי\s+(אתם\s+)?(פתוחים|פותחים|סגורים|עובדים)|"
    r"אתם\s+(פתוחים|עובדים|סגורים)|פתוחים\s+(עכשיו|היום|כרגע)|"
    r"(פתוח|סגור)\s+(עכשיו|היום|כרגע)|"
    r"open\s+now|opening\s+hours|are\s+you\s+open|what\s+time.*open")


def _is_hours_intent(low: str) -> bool:
    if _HOURS_INTENT_RE.search(low):
        return True
    # הודעה קצרה שכולה 'פתוחים'/'פתוח'/'סגורים' (זכר) = שאלת שעות. לא תופס מוצר
    # ('אוזניות פתוחות' = נקבה, ולא הודעה בודדת).
    bare = low.strip().rstrip("?؟ ").strip()
    return bare in {"פתוחים", "פתוח", "אתם פתוחים", "פתוחים עכשיו", "סגורים", "אתם עובדים"}


def _answer_hours(phone):
    """מענה ישיר לשאלת שעות פעילות (לא חיפוש מוצר)."""
    hours = ("שעות הפעילות:\nימים א׳-ה׳ | 09:00-21:00\nיום ו׳ | 09:00-14:00")
    if _within_business_hours():
        wa.send_text(phone, f"🟢 כן, אנחנו פתוחים עכשיו!\n\n{hours}\n\nנשמח לעזור בכל שאלה 🙂")
    else:
        wa.send_text(phone, f"🕐 כרגע אנחנו סגורים.\n\n{hours}\n\nמוזמנים להשאיר כאן הודעה ונחזור אליכם בהקדם 🙂")
