...
def build_targets(cat=None, beat=None) -> list:
    ...
    # בדיוק בזמן הסריקה. בלי הגארד הזה **כל** המוצרים מסומנים "לא בפיד",
    # הצ׳יפים קורסים לשניים, והתמונה הטובה נדרסת (אסי, 28/07/2026 — "לאן
    # נעלמו כל הצ׳יפים?"). מצב כזה הוא תקלה, לא מציאות.
    if catalog and not feed:
        logger.warning("zap: הפיד ריק והקטלוג מלא (%d) — תחזוקה? מבטלים את הסריקה",
                       len(catalog))
        return []
...
def _reason(row: dict, cat=None) -> tuple:
    ...
    if not row.get("in_feed"):
        ...
        if not row.get("site_instock", 1):
            ...
            if pos:
                # ⚠️ צ׳יפ נפרד, ובכוונה: זו אינה בעיית זאפ אלא בעיית **מכירה**.
    ...
    # ⚠️ פער הכותרת מחושב **בסריקה** ולא רק בלחיצה על "מה צריך": הצ׳יפים
...
def _model_tokens(s: str) -> set:
    ...
    # ⚠️ "Fold 8" ו-"Fold8" הם אותו דגם. בלי האיחוד הזה כל צד מייצר אסימון אחר
    # (או אף אחד) ושני שמות זהים נראים כדגמים שונים.
    low = re.sub(r"\b([a-z]{2,})\s+(\d{1,4})\b", r"\1\2", low)
    # ⚠️ תקרת 2 אותיות ("A57", "X9") לא תפסה שמות-דגם מודבקים ארוכים: ל-
    # "Galaxy Z Fold8" יצא **אפס** אסימונים, והכלל "אין אסימונים ⇒ לא ראיה נגד"
    # אישר כל טלפון סמסונג אחר — Fold8 הוצמד לדף של Fold6, ו-Fold8 Ultra לדף של
    # S26 Ultra (אסי, 05/08/2026). שם דגם מודבק הוא בדיוק מה שמבדיל כאן.
    toks = set(re.findall(r"\b([a-z]{0,10}\d{1,4}[a-z]?)\b", low))
...
def _audio_query(name: str) -> str:
    ...
    # ("FreeClip 2 Bluetooth", שנענש על מילה עודפת) — היינו היחידים על
    # הדף הנכון והכלי הצביע על השני (אסי, 28/07/2026). אלה שני מוצרים
    # שונים: ה-2S מגיע אלינו בעוד שבוע, וצריך שיתפוס את הדף שלו.
    q = re.sub(r"\s*[\(\)]\s*", " ", q)
    q = re.sub(r"\s+", " ", q).strip(" -–—,.")
    q = re.sub(r"\b(\d+)\s+([A-Za-z])\s*$", r"\1\2", q)
    return q
